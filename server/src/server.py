...
class OTAService(ota_pb2_grpc.FirmwareUpdateServiceServicer):
	...
	def CheckForUpdate(self, request, context):
		try:
			print(f"[Log] CheckForUpdate ping from: {request.device_id} (v{request.current_version})")
			print("[Tracer] 1. Attempting to write to database...")
			database.update_device_status(request.device_id, request.current_version)
			print("[Tracer] 1. Database write successful!")	
			print("[Tracer] 2. Querying active campaigns...")
			target_version = database.get_active_campaign_for_device(request.device_id)
			print(f"[Tracer] 2. Query successful! Target version is: {target_version}")	
			response = ota_pb2.UpdateResponse()

			if target_version and request.current_version != target_version:
				print(f"[Campaign] Matched! {request.device_id} is authorized for v{target_version}")
				...
				print(f"[Campaign] No active updates required for {request.device_id}")
				response.update_available = False
			print("[Tracer] 3. Returning response to client.")
			return response
		except Exception as e:
			import traceback
			print(f"[CRITICAL TRACEBACK in CheckForUpdate]: {e}")
			traceback.print_exc() # This forces Python to print the exact line number!
			return ota_pb2.UpdateResponse(update_available=False)

	def DownloadUpdate(self, request, context):
		print(f"[Log] Streaming firmware to: {request.device_id}")
		offset = request.resume_offset
		try:
			with open(FIRMWARE_PATH, "rb") as f:
				if offset>0:
					print(f"[Network] Client requested resume from byte {offset}. Seeking ...")
					f.seek(offset)
				...
				while True:
					...
					if offset == 0 and chunk_count>80:
						print("[network Simulation] Car entered tunnel! Dropping connection.")
						context.abort(grpc.StatusCode.UNAVAILABLE, "Network connection lost.")
				
			print(f"[Log] Finished streaming to {request.device_id}")
		except Exception as e:
			print(f"[Error] {e}")
...
def serve():
	...
	try:
		...
		print(f"[Security] TLS/SSL enabled on port {SERVER_PORT}")
        
	except FileNotFoundError:
		print("[CRITICAL] Missing SSL certificates. Cannot start secure server.")
		return

	server.start()
	print(f"Secure OTA Server started on {SERVER_PORT}")
